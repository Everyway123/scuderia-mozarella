...
test('E11: радар погоди попереджає ДО дощу, і превентивний заїзд працює', async ({ page }) => {
  ...
  // Регресія: коли обидві машини ВЖЕ на інтері, реактивний запит «пішов дощ,
  // міняємо гуму?» — знущання, а не рішення. Він не має зʼявитись узагалі.
  await page.click('#speedBtn');
  const nagging = await waitForPrompt(page, 'rain', 25);
  expect(nagging, 'дощовий запит зʼявився, хоча машини вже на дощовій гумі').toBe(false);
